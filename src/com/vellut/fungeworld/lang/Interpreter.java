...
public class Interpreter {
	...
	private boolean runInstruction(Instruction instr) {
		...
		case SPAWN: {
			...
			int counter = (Integer) instr.getAttachedData();
			counter--;
			if(counter == 0) {
				// In order to prevent infinite spawns (which slows down the 
				// simulation to a crawl)
				// Replace the spawn by random instruction
				boardProxy.mutate(instructionPointer, instr);
			} else {
				instr.setAttachedData(counter);
			}
		}
			break;
		case REL_DPOS_LEFT: {
